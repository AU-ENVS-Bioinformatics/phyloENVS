...
This vignette demonstrates how phyloENVS generalize to mRNA data. We will be using mRNA read counts from ice samples collected in Qaanaaq, Greenland, similar to the vignette "example_1". We focus on glacier samples collected along two transects. The sampling locations along each transect were categorized into three zones based on wetness. Furthermore, we distinguish between samples facing east (E) and west (W). 
...
We perform Non-metric Multidimensional Scaling (NMDS).

```{r, message=FALSE, fig.width=12, fig.height=5, out.width="80%", fig.align="center"}
...
In the ordination, the wet samples seemed to be different from the rest. Especially for the PCyc data, the direction might also explain the differences in mRNA expression. We look into the relative abundances for PCyc reads. 
